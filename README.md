## Flutter + MVC +Refresh 极简的开发体验

在上一个项目开发的时候，产出了 [flutter_allroundrefresh 插件](https://www.jianshu.com/p/6148062cbb86) （**集下拉刷新、上拉加载、初始转圈、错误页面、异常重试 一条龙功能**）极大的提高了当时的开发效率，该项目结束后，沉淀下来后，发现这个小插件还是有很多不足的地方，比如每个页面都需要复制一堆代码，耦合度较高（接口格式）等问题。

尤其是接口结果集格式高耦合的问题，考虑到现在用flutter作为生产环境的公司（可能估计大概也许瞎猜的）还是相对较少，估计更多的应该是移动开发者、前端、以及学生过来的（试水、学习），这种情况就没有那么多正式接口，而大部分则是网络上的公共API，这样的API结果集格式花样百出，这样的话该插件内部封装的高耦合数据解析方案反而成了她走向大众化插件的绊脚石。

**那么能不能降低这个耦合，又保留flutter_allroundrefresh上功能呢？于是，苦思冥想后便有了这篇文章中的新UI插件，flutter_futrue。（低耦合，更简洁，定制接口解析方案。）**

1.**这里需要先提一下MVC架构模式。**
笔者在架构之初，复习了一遍MVC、MVP、MVVM架构模式以及flutter上流行的状态管理BLoC、Redux，最后根据Flutter自身的特点，研究出了Flutter + MVC +REFRESH的简化UI的方案。
...
在笔者架构中指当前page页面，或者说页面对应的buildBody（ListView、GridView）及其他buildXXX方法。

>**Controller（控制层）**是对数据的接收和触发事件的接收和传递。
在笔者架构中指BaseState极其子类，这里的Controller会对数据进行初步处理，用于处理数据的非正常情况的逻辑，
...
3.2具体页面的使用（这里就需要继承BaseStateForLocalData了）
```
import 'dart:async';
import 'package:flutter/material.dart';
import 'package:flutter_futrue_example/dao/simple_dao.dart';
